...
영상이 끝까지 재생되면 유튜브의 실제 "연관 동영상"을 찾아 자동으로 이어서 재생합니다. 다음 영상이 정해지면 현재 재생 화면에 "다음 자동재생: 제목 · 채널"이 잠시 표시됩니다. `a`로 끄거나 `q`로 직접 종료하면 새 검색어 입력 프롬프트로 돌아옵니다. `Enter`만 누르면 종료.
